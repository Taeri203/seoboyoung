...
export async function POST(request: Request) {
  ...
  try {
    response = await fetch(BREVO_ENDPOINT, {
      ...
      body: JSON.stringify({
        sender: { name: "서보영.kr 유권자의 소리", email: fromEmail },
        to: [{ email: toEmail }],
        subject: `[서보영.kr 유권자의 소리] ${title}`,
...
function getBrevoErrorMessage(status: number, errorText: string) {
  ...
  if (lower.includes("unrecognised ip") || lower.includes("unrecognized ip")) {
    return "Brevo Authorized IP 설정을 확인해 주세요.";
  }
...
function renderEmail(data: Record<string, string>) {
  ...
  return `
    <div style="font-family:Arial,'Apple SD Gothic Neo','Noto Sans KR',sans-serif;color:#101828;line-height:1.6">
      <h1 style="color:#11205A">서보영.kr 유권자의 소리</h1>
