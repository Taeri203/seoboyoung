...
export function GenerationCard({ generation, index = 0 }: { generation: Generation; index?: number }) {
  ...
  return (
    <article className={`h-full overflow-hidden rounded-[2rem] border shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${tone}`}>
      {generation.image ? (
        <div className="bg-[#FFF9E8] p-2 md:hidden">
          <div className="rounded-[1.4rem] bg-white">
            <img
              src={generation.image}
              alt={`${generation.audience} 공약 이미지`}
